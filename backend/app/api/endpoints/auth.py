...
@inject
def sign_in(user_info: SignIn, service: AuthService = Depends(Provide[ApplicationContainer.services.auth_service])):
    return service.sign_in(user_info)


@router.post("/sign-up", response_model=UserBasicResponse)
@inject
def sign_up(user_info: SignUp, service: AuthService = Depends(Provide[ApplicationContainer.services.auth_service])):
