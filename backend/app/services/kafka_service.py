...
class KafkaService:
        ...
        while retries < max_retries:
            try:
                ...
                self._logger.warning(
                    f"Kafka brokers not available at {self.bootstrap_servers}, retry {retries + 1}/{max_retries}")
                retries += 1
    ...
    def _buffer_notification(self, user_id: str, notification: Dict[str, Any]):
        ...
        with self.notification_buffer_lock:
            ...
            self._logger.info(
                f"Buffered notification for offline user {user_id}, buffer size: {len(self.notification_buffer[user_id])}")
    ...
    async def send_notification(self, user_id: str, notification: Dict[str, Any]):
        ...
        try:
            ...
            if websocket.client_state == WebSocketState.DISCONNECTED:
                self._logger.warning(
                    f"WebSocket for user {user_id} is disconnected, buffering notification and removing connection")
                self._buffer_notification(user_id, notification)
            ...
            if "websocket operation outside of a connection" in str(e).lower():
                self._logger.warning(
                    f"WebSocket for user {user_id} is no longer connected, buffering notification and removing connection")
                self._buffer_notification(user_id, notification)
                self.remove_connection(user_id)
            else:
